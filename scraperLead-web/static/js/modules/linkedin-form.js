...
export function initLinkedInForm() {
  ...
  if (importToggle) {
    importToggle.addEventListener('click', () => {
      ...
      importChevron.textContent = importBody.classList.contains('hidden') ? '▼' : '▲';
    });
  }
  ...
  if (importBtn) {
    importBtn.addEventListener('click', async () => {
      ...
      importAlert.className = 'hidden rounded-xl px-4 py-3 text-sm mt-3';
      importAlert.textContent = '';

      if (!username) {
        importAlert.className = 'rounded-xl px-4 py-3 text-sm mt-3 bg-red-50 border border-red-200 text-red-700';
        importAlert.textContent = 'Introduce un nombre de cuenta.';
        return;
      }
      if (!cookiesJson) {
        importAlert.className = 'rounded-xl px-4 py-3 text-sm mt-3 bg-red-50 border border-red-200 text-red-700';
        importAlert.textContent = 'Pega el JSON de cookies exportado con Cookie-Editor.';
        return;
      }
      ...
      try {
        const res = await fetch(`${window.__BASE__}/api/linkedin/accounts/import-session`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            username,
            cookies_json: cookiesJson,
            display_name: importName.value.trim() || username,
          }),
        });
        const data = await res.json();
        if (res.ok) {
          importAlert.className = 'rounded-xl px-4 py-3 text-sm mt-3 bg-green-50 border border-green-200 text-green-700';
          importAlert.textContent = data.message || 'Sesión importada correctamente.';
          importCookies.value = '';
          ...
          importAlert.className = 'rounded-xl px-4 py-3 text-sm mt-3 bg-red-50 border border-red-200 text-red-700';
          importAlert.textContent = data.detail || 'Error al importar la sesión.';
        }
      } catch (e) {
        importAlert.className = 'rounded-xl px-4 py-3 text-sm mt-3 bg-red-50 border border-red-200 text-red-700';
        importAlert.textContent = `Error de red: ${e.message}`;
      } finally {
